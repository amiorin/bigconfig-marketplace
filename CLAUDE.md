...
- **`Caddyfile`** — Reverse proxies `/api*` and `/_*` to PocketBase (`:8090`), everything else to the Astro dev server (`:4321`). This is dev-only; in production, PocketBase serves `pb_public` directly on `:8090`.
- **`Procfile`** — Runs `pb`, `web` (astro dev), and `caddy` together via `hivemind`.
- **`Dockerfile`** — Two stages: build the Astro site with `PUBLIC_PB_URL` baked in (build-time arg), then assemble an Alpine image with the PocketBase binary, migrations, hooks, and the built static site.
...
`PUBLIC_PB_URL` must be set at build time for the web app (it's inlined into the static bundle). `.envrc` provides local defaults via direnv.
